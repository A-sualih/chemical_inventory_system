...
const TruckIcon = ({ className }) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 18.75a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 0 1-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 0 0-3.213-9.193 2.056 2.056 0 0 0-1.58-.806H14.25M16.5 18.75h-2.25m0-11.25v11.25m0-11.25H8.25m0 11.25H3.375a1.125 1.125 0 0 1-1.125-1.125V11.25a9 9 0 0 1 9-9h1.5a2.25 2.25 0 0 1 2.25 2.25v.75" />
  </svg>
);
...
const Batches = () => {
  ...
  const getStatusStyle = (status) => {
    switch (status) {
      ...
      case 'Recalled': return 'bg-secondary-900 text-white border-secondary-800';
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-20">
      <div className="bg-white border-b border-secondary-200 sticky top-0 z-30 px-6 py-4">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-secondary-500 font-medium">Traceability and lifecycle management for chemical lots</p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="relative group">
              ...
              <input 
                type="text" 
                placeholder="Search batch, lot #, chemical..." 
                value={search}
                onChange={e => setSearch(e.target.value)}
                className="pl-10 pr-4 py-2.5 bg-secondary-50 border border-secondary-200 rounded-xl text-sm focus:ring-4 focus:ring-primary-500/10 focus:border-primary-400 transition-all md:w-80 outline-none"
              />
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-8">
        <div className="bg-white rounded-[32px] border border-secondary-200 shadow-xl overflow-hidden">
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-secondary-50/50">
                  <th className="p-6 text-[11px] font-black text-secondary-500 uppercase tracking-widest">Batch Identity</th>
                  <th className="p-6 text-[11px] font-black text-secondary-500 uppercase tracking-widest">Chemical & Supplier</th>
                  <th className="p-6 text-[11px] font-black text-secondary-500 uppercase tracking-widest">Quantity & Units</th>
                  <th className="p-6 text-[11px] font-black text-secondary-500 uppercase tracking-widest">Safety Clock</th>
                  <th className="p-6 text-[11px] font-black text-secondary-500 uppercase tracking-widest">Containers</th>
                ...
                {filteredBatches.map((batch) => (
                  <tr key={batch._id} className="group hover:bg-secondary-50/30 transition-colors">
                    <td className="p-6">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-white border-2 border-secondary-100 rounded-2xl flex items-center justify-center flex-shrink-0 group-hover:border-primary-200 group-hover:bg-primary-50 transition-all">
                          <TagIcon className="w-6 h-6 text-secondary-400 group-hover:text-primary-500" />
                        </div>
                        <div>
                          <div className="text-sm font-black text-secondary-900">{batch.batch_number}</div>
                          <span className={`inline-block px-1.5 py-0.5 rounded text-[9px] font-black uppercase border mt-1 ${getStatusStyle(batch.status)}`}>
                            {batch.status}
                          </span>
                        </div>
                      </div>
                    ...
                    <td className="p-6">
                      ...
                      <div className="flex items-center gap-1.5 mt-1 text-[11px] text-secondary-400 font-medium">
                    ...
                    <td className="p-6">
                      <div className="flex flex-col gap-1.5">
                        <div className="flex items-center gap-2 text-xs font-bold text-secondary-700">
                          <CalendarDaysIcon className="w-3.5 h-3.5 text-secondary-400" />
                          EXP: {batch.expiry_date ? new Date(batch.expiry_date).toLocaleDateString() : 'N/A'}
                        </div>
                        <div className="w-24 h-1 bg-secondary-100 rounded-full overflow-hidden">
                           <div className={`h-full ${batch.status === 'Expired' ? 'w-full bg-red-500' : 'w-2/3 bg-green-500'}`}></div>
                        </div>
                      </div>
                    </td>
                    <td className="p-6">
                       <div className="flex -space-x-2">
                        {batch.container_ids?.slice(0, 3).map((id, i) => (
                          <div key={i} className="w-7 h-7 rounded-full bg-secondary-100 border-2 border-white flex items-center justify-center text-[8px] font-black text-secondary-400" title={id}>
                            <ArchiveBoxIcon className="w-3.5 h-3.5" />
                          </div>
                        ))}
                        {batch.container_ids?.length > 3 && (
                          <div className="w-7 h-7 rounded-full bg-primary-50 border-2 border-white flex items-center justify-center text-[8px] font-black text-primary-600">
                            +{batch.container_ids.length - 3}
                          </div>
                        )}
                        {batch.container_ids?.length === 0 && (
                          <span className="text-[10px] text-secondary-300 italic font-bold">No containers</span>
                        )}
                      </div>
                    </td>
